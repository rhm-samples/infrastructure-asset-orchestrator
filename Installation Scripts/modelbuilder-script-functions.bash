...
function checkClusterServiceVersionSucceeded() {
        ...
        retryCount=20
        retries=0
        check_for_csv_success=$(oc get csv -n "$projectName" --ignore-not-found | awk '$1 ~ /ibm-modelbuilder-for-vision/ { print }' | awk -F' ' '{print $NF}')
...
function checkDeploymentStatus() {
        ...
        retryCount=70
        retries=0
        check_for_deployment_status=$(oc get csv -n "$projectName" --ignore-not-found | awk '$1 ~ /ibm-modelbuilder-for-vision/ { print }' | awk -F' ' '{print $NF}')
        until [[ $retries -eq $retryCount || $check_for_deployment_status = "Ready" || $check_for_deployment_status = "Failed" ]]; do
                sleep 30
                check_for_deployment_status=$(oc get Modelbuilder modelbuilder-demo --output="jsonpath={.status.phase}")
                retries=$((retries + 1))
        done
